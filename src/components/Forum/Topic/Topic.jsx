...
function Topic() {
  ...
  const fetchTopic = () => {
    ...
    try {
      ...
      axios.post("http://localhost:3001/comments/", { topicIdParam }).then(res => {
        setComments(() => res.data.comments);
      });

      // Get Topics
      axios.get('http://localhost:3001/topic/' + topicIdParam)
        .then(res => {
  ...
  const Submit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3001/topic/comment/new', {
        comment,
        topicParam
      }).then(res => {
